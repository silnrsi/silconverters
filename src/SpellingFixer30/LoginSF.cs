...
namespace SpellingFixer30
{
    ...
    internal class LoginSF : System.Windows.Forms.Form
	{
        ...
        internal const string    RegistryCscRootKey = @"SOFTWARE\SIL\ConsistentSpellingFixer";
        ...
        private string  RegistryKeyMostRecentProject = "MostRecentProject";
		...
		public LoginSF()
		{
            ...
            EncConverters myECs = aECs.FilterByProcessType(SpellingFixer.SFProcessType);
            ...
            try
            {
                var keyLastSFProject = Registry.LocalMachine.OpenSubKey(RegistryCscRootKey);
                nIndex = checkedListBoxProjects.FindString((string)keyLastSFProject.GetValue(RegistryKeyMostRecentProject));
            }
            catch {}
            ...
            this.textBoxWordBoundaryDelimiter.Text = SpellingFixer.cstrDefaultWordBoundaryDelimiter;
        ...
        {
            return SpellingFixer.cstrSFConverterPrefix + strProjName;
        }

        internal string PartialName(string strFullName)
        {
            int nPrefixLen = SpellingFixer.cstrSFConverterPrefix.Length;
            if(     (strFullName.Length > nPrefixLen)
                &&  (strFullName.Substring(0, nPrefixLen) == SpellingFixer.cstrSFConverterPrefix) )
        ...
        internal static void CreateCCTable(StreamWriter sw, string strEncConverterName, string strPunctuation, string strCustomCode, bool bUnicode)
        {
            ...
            sw.WriteLine(String.Format("c This cc table was created by SpellingFixer.dll v{0} on {1}.",strVersion, DateTime.Now.ToShortDateString()));
        ...
        internal static string GetMapTableFolderPath
        {
            get
            {
                RegistryKey keySilEncConverters = Registry.LocalMachine.OpenSubKey(@"SOFTWARE\SIL\SilEncConverters22");
                string strMapsTableDir = (string)keySilEncConverters.GetValue("RootDir");
                if (strMapsTableDir == null)
                {
                    // this shouldn't happen, but might in the debug/development environment
                    strMapsTableDir = Environment.GetFolderPath(Environment.SpecialFolder.CommonProgramFiles);
                    strMapsTableDir += @"\SIL\";
                }
                else if (strMapsTableDir.Substring(strMapsTableDir.Length - 1, 1) != @"\")
                    strMapsTableDir += @"\";
                strMapsTableDir += "MapsTables";
        ...
        private void buttonAddNewProject_Click(object sender, System.EventArgs e)
        {
            ...
            {
                MessageBox.Show("Can't use the double-quote character for the word boundary delimiter",SpellingFixer.cstrCaption);
                return;
            }
            ...
            if( aEC != null )
            {
                ...
                {
                    if( MessageBox.Show(String.Format("A project already exists by the name {0}. Click 'Yes' to overwrite", this.textBoxNewProjectName.Text),SpellingFixer.cstrCaption, MessageBoxButtons.YesNoCancel) == DialogResult.Yes)
                    {
                ...
                {
                    if( MessageBox.Show(String.Format("Do you want to update the '{0}' project?", this.textBoxNewProjectName.Text),SpellingFixer.cstrCaption, MessageBoxButtons.YesNoCancel) == DialogResult.Yes)
                    {
                        // take it out of the check box list
                        checkedListBoxProjects.Items.Remove(strPartialName);
            ...
            if( strCCTableSpec == null )
            {
                ...
                if( File.Exists(strCCTableSpec) )
                {
                    ...
                    if( MessageBox.Show(String.Format("A file exists by the name {0}. Click 'Yes' to overwrite", strCCTableSpec),SpellingFixer.cstrCaption, MessageBoxButtons.YesNoCancel) == DialogResult.Yes)
                    {
                        File.Delete(strCCTableSpec);
#if WriteOnAdd
...
//  a non-trapable error if there are no rules in the file. So just create it when it is actually needed
            ...
            if(bRewriteCCTable)    // we are going to continue using the old file... so we must re-write it.
            {
                ...
                DataTable   myTable;
                if( SpellingFixer.InitializeDataTableFromCCTable(strCCTableSpec, enc, m_strWordBoundaryDelimiter, out myTable) )
                {
                    ReWriteCCTableHeader(strCCTableSpec,m_strNonWordCharacters,enc, myTable.Rows.Count > 0);
        ...
        internal bool LoadProject(string strProjectName)
        {
            ...
            if( aEC != null )
            {
                ...
                RegistryKey keyLastSFProject = Registry.LocalMachine.OpenSubKey(RegistryCscRootKey);
                keyLastSFProject.SetValue(RegistryKeyMostRecentProject,strProjectName);
                return true;
            }
